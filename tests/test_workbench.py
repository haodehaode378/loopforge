...
class WorkbenchTests(unittest.TestCase):
    def test_workbench_snapshot_reads_projects_runs_and_reports(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("execution_adapter", done_run["approval"])
            self.assertEqual(done_run["approval"]["execution_gate"]["executable_actions"], [])
            self.assertEqual(done_run["approval"]["execution_adapter"]["executable_actions"], [])
            self.assertTrue(done_run["approval"]["execution_adapter"]["dry_run_only"])
            ...
            self.assertEqual(done_run["reviewer_status"]["state"], "changes-requested")
            self.assertTrue(done_run["reviewer_status"]["ready_for_next_loop"])
            self.assertFalse(done_run["reviewer_status"]["execution_authority"])
            ...
            self.assertIn("Reviewer Status", done_run["sections"])
            ...
            self.assertIn("Execution adapter contract", html)
            ...
            self.assertIn("Reviewer status", html)
            self.assertIn("ready_for_next_loop", html)
