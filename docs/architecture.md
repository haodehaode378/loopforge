...
- Approval ledger entries now include local actor identity, signature payload hashes, a placeholder signature algorithm, and status evaluation. Placeholder signatures are deterministic audit scaffolding only; they are not cryptographic proof of identity.
